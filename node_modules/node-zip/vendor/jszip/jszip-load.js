...
(function () {
   ...
   StreamReader.prototype = {
      ...
      {
         if (this.stat.size < newIndex || newIndex < 0)
         {
            throw new Error("End of stream reached (stream length = " +
                            this.stat.size+ ", asked index = " +
      ...
      eof : function ()
      {
         return this.index >= this.stat.size;
      },
      /**
      ...
      byteAt : function(i)
      {
         ...
         fs.readSync(this.stream, buf, 0, 1, i);
      ...
      readInt : function (size)
      {
         ...
         fs.readSync(this.stream, buf, 0, size, this.index)
         switch(size) {
            case 2:
            ...
            break;
            case 8:
            result = (buf.readUInt32LE(0)<<32) + buf.readUInt32LE(4);
            break;
         }
         this.index += size;
         return result;
   ...
   ZipEntries.prototype = {
      ...
      readLocalFile : function( file )
      {
            ...
            this.checkSignature(JSZip.signature.LOCAL_FILE_HEADER);
            file.readLocalPart(this.reader);
            return file.uncompressedFileData;
      },
      /**
      ...
      readEndOfCentral : function()
      {
         ...
         // {
            this.zip64 = false;
            // find the offset 
            var buf = new Buffer(1024 + 4);

            var foffset = fs.fstatSync(this.reader.stream).size - 1024 - 4;
            while(true) {
               var readBytes = fs.readSync(this.reader.stream, buf, 0, 1024 + 4, foffset); 
               var offset = buf.toString("binary").lastIndexOf(JSZip.signature.CENTRAL_DIRECTORY_END);
               if ( offset !== -1 ) {
                  offset += foffset;
                  break;
               }
   ...
   JSZip.prototype.load = function(data, options)
   {
      ...
      this.zipEntries = new ZipEntries(data, options);
      /*
